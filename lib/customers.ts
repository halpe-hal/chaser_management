...
// 入会（いずれかのプラン）より前の段階。ここへ戻された場合は「間違えて入会/退会にしてしまった」の訂正とみなす
const BEFORE_JOIN_STATUSES: readonly CustomerStatus[] = [
  "未来店",
  "検討",
  "事前キャンセル",
  "無断キャンセル",
...
export interface StatusFlagUpdates {
  ever_cancelled_at?: string | null;
  ever_rebooked_at?: string | null;
  ever_joined_at?: string | null;
}

// ステータス変更のたびに呼ぶ。再予約率・退会率を「今のステータス分布」ではなく「一度でもそのステータスに
// なった人のうち、一度でも次の段階に進めた人の割合」として安定して集計できるよう、変化のあった分だけ返す。
// 前段ステータスへ戻された場合はステータス訂正とみなしてフラグを解除する（誤操作の巻き戻し救済）。
export function computeStatusFlagUpdates(oldStatus: CustomerStatus, newStatus: CustomerStatus): StatusFlagUpdates {
  if (oldStatus === newStatus) return {};
  const now = new Date().toISOString();
  const updates: StatusFlagUpdates = {};
  ...
  if (isJoinedStatus(newStatus)) {
    updates.ever_joined_at = now;
  } else if ((isJoinedStatus(oldStatus) || oldStatus === "退会") && BEFORE_JOIN_STATUSES.includes(newStatus)) {
    updates.ever_joined_at = null;
  }
...
export type StatusCounts = Record<CustomerStatus, number> & {
  ...
  everJoined: number; // 一度でも入会になった人数（退会率の分母）
};
...
export async function getStatusCounts(storeId: number | null, filters: StatusCountsFilters = {}): Promise<StatusCounts> {
  const supabase = await createClient();
  let query = supabase.from("customers").select("status, ever_cancelled_at, ever_rebooked_at, ever_joined_at");
  ...
  const rows = (data ?? []) as Pick<Customer, "status" | "ever_cancelled_at" | "ever_rebooked_at" | "ever_joined_at">[];
  return buildStatusCounts(rows);
}

// 手元にある顧客一覧（例: 特定日の予約分）から、DBに問い合わせ直さずにステータス件数を集計する
export function buildStatusCounts(
  customers: (Pick<Customer, "status"> &
    Partial<Pick<Customer, "ever_cancelled_at" | "ever_rebooked_at" | "ever_joined_at">>)[]
): StatusCounts {
  ...
  for (const c of customers) {
    ...
    if (c.ever_joined_at) counts.everJoined++;
    counts[c.status]++;
  }
  return counts;
}
...
export async function getStaffJoinRatesByStaff(
  ...
  const supabase = await createClient();
  let query = supabase.from("customers").select("status, staff_member_id, ever_joined_at");
  if (storeId !== null) query = query.eq("store_id", storeId);
  if (filters.dateFrom) query = query.gte("reservation_date", filters.dateFrom);
  if (filters.dateTo) query = query.lte("reservation_date", filters.dateTo);
  ...
  const { data, error } = await query;
  if (error) throw error;
  const rows = (data ?? []) as { status: CustomerStatus; staff_member_id: number | null; ever_joined_at: string | null }[];

  const byStaff = new Map<number | null, { status: CustomerStatus; ever_joined_at: string | null }[]>();
  for (const row of rows) {
    if (!byStaff.has(row.staff_member_id)) byStaff.set(row.staff_member_id, []);
    byStaff.get(row.staff_member_id)!.push({ status: row.status, ever_joined_at: row.ever_joined_at });
  }
...
export interface DashboardStats {
  ...
  visitedCount: number; // 来店率の分子と同じ人数（検討＋入会全プラン＋退会＋見込みなし（来店済））
  memberCount: number; // 現在の会員数（入会ステータス（２年・1年・通常）の合計。退会した人は含まない）
  withdrawnCount: number; // 現在「退会」の人数
  ...
  withdrawalRate: number | null; // 0-100（対象0件のときは null）
}
...
// 再予約率・入会率（の分子）・退会率は、現在のステータス分布ではなく ever_*_at フラグで算出する。
// 一度でも事前キャンセル/無断キャンセルになった人のうち、一度でも再予約済になれた人の割合（再予約率）。
// 一度でも入会した人のうち、一度でも退会した人の割合（退会率）。
// こうすることで、その後ステータスが入会・退会・見込みなし等に進んでも率が変動しない（computeStatusFlagUpdates参照）。
// 来店した人数（検討＋入会全プラン＋退会＋見込みなし（来店済））と、そのうち一度でも入会した人数を求める
function countVisitedAndJoined(counts: StatusCounts): { visited: number; joined: number } {
  ...
  return { visited: visitDenominator - didNotVisit, joined: counts.everJoined };
}
...
export function computeDashboardStats(counts: StatusCounts): DashboardStats {
  ...
  const memberCount = JOINED_STATUSES.reduce((sum, s) => sum + counts[s], 0);
  const withdrawnCount = counts["退会"];
  ...
  return {
    ...
    withdrawalRate: counts.everJoined > 0 ? (withdrawnCount / counts.everJoined) * 100 : null,
  };
}
