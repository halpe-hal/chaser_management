...
export function BaseDateResetForm({ baseDateResetDate }: { baseDateResetDate: string | null }) {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-black/5 p-6">
      <h2 className="text-lg font-semibold text-gray-900 mb-1">グランドオープン日</h2>
      <p className="text-sm text-gray-500 mb-4">
        この日を過ぎると、ご予約日がこの日より前の顧客は、追客ステップの起算日がグランドオープン日に切り替わります（例：プレオープン期間中に登録した顧客の起算日を、実際のオープン日に統一する）。ご予約日がこの日以降の顧客には影響しません。
        また、下のステータス編集で「プレオープン期間」を選んで追加したステップは、このグランドオープン日を過ぎると自動的に使われなくなり、「グランドオープン以降」のステップに切り替わります。空欄にすると無効になります（全店舗共通の設定です）。
      </p>
      <form action={formAction} className="flex flex-wrap items-end gap-3">
        {state?.error && <p className="w-full text-sm text-red-600">{state.error}</p>}
        {state?.success && <p className="w-full text-sm text-emerald-700">保存しました。</p>}
        <div>
          <label className="block text-xs text-gray-500 mb-1">グランドオープン日</label>
          <input
            type="date"
